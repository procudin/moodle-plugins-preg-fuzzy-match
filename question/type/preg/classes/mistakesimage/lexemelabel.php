 ...
 */
class qtype_preg_lexeme_label extends qtype_preg_abstract_label {
    /**
    ...
    public function __construct($text) {
        $bbox = qtype_preg_get_text_bounding_box($text);
    ...
    public function recompute_size() {
        ...
        for($i = 0; $i < count($operationspairs); $i++) {
            ...
            if ($pair[1] == 'transpose') {
                ...
                $bbox = qtype_preg_get_text_bounding_box($pair[0]);
                $fbbox = qtype_preg_get_text_bounding_box($firstletter);
                $sbbox = qtype_preg_get_text_bounding_box($secondletter);
            ...
            if ($i != count($operationspairs) - 1) {
                if (in_array($operationspairs[$i][1], $sametext) && in_array($operationspairs[$i + 1][1], $sametext)) {
                    $width += qtype_preg_compute_kerning($operationspairs[$i][0], $operationspairs[$i + 1][0]);
                }
            }
        }
    ...
    public function paint(&$im, $palette, $currentrect, $bottom) {
        ...
        for($i = 0; $i < count($operationpairs); $i++) {
            ...
            if ($operationpair[1] != 'insert' && $operationpair[1] != 'missing_separator') {
                qtype_preg_render_text($im, $x, $currentrect->y + $this->baselineoffset, $operationpair[0], $currentcolor);

                $bbox = qtype_preg_get_text_bounding_box($operationpair[0]);
                ...
                if ($i != count($operationpairs) - 1) {
                    if (in_array($operationpairs[$i][1], $sametext) && in_array($operationpairs[$i + 1][1], $sametext)) {
                        $nx += qtype_preg_compute_kerning($operationpairs[$i][0], $operationpairs[$i + 1][0]);
                    }
                }
            ...
            if ($operationpair[1] == 'insert') {
                if (core_text::strlen($operationpair[0]) == 1) {
                    ...
                    qtype_preg_render_text($im, $x + 2 * TINY_SPACE, $currentrect->y + $this->baselineoffset - $metrics['height'] + 2 * TINY_SPACE, $operationpair[0], $palette['red']);
                    $middle = $x + ($nx - $x) / 2;
